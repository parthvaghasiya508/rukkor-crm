...
import { getClusters, deleteCluster } from '../redux/actions/cluster';
...
export class Clusters extends Component {
  ...
  paginate = (pageNumber) => (e) => {
    ...
    this.props.getClusters(pageNumber,search_keyword);
  };
  ...
  handleEdit = (id) => (e) => {
    this.props.history.push(`/edit-cluster/${id}`);
  };
  ...
  confirmDelete = (e) => {
    this.props.deleteCluster(this.state.selectedId,this.props);
  ...
  searchData = (e) => {
    ...
    this.props.getClusters(page,search_keyword);
  };
  render() {
    return (
      <>
              ...
              <div className="card-body">
                <div className="table-responsive">
                  ...
                  >
                    ...
                    <tbody>
                      {this.props.clusters && this.props.clusters.length > 0 ? (
                        this.props.clusters.map((cluster, index) => (
                          <tr key={cluster._id}>
                            <td>{cluster.title}</td>
                            <td>{cluster.status_text}</td>

                            <td className="text-center">
                              <i
                                onClick={this.handleEdit(cluster._id)}
                            ...
                            <td className="text-center">
                              <i
                                onClick={this.handleDelete(cluster._id)}
                ...
                {this.props.clusters.length > 0 && this.props.pagination ? (
                  <>
...
const mapStateToProps = state => ({ clusters: state.cluster.list, pagination: state.cluster.pagination });
const mapDispatchToProps = {
  getClusters,deleteCluster
};
